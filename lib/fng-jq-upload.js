...
function Controller(fng, processArgs, options) {
    ...
    fng.app.get.apply(fng.app, processArgs(modifiedOptions, ['file/:model/thumbnail/:id', function (req, res) {
            try {
                var mongo_1 = fng.mongoose.mongo;
                var model = req.params.model;
                var resource_1 = fng.getResource(model);
                var id_1 = req.params.id;
                var idObj_1;
                fng.mongoose.connection.db.collection(resource_1.model.collection.collectionName + '.files', function (err, files) {
                    if (err) {
                        throw err;
                    }
                    files.find({ 'metadata.original_id': id_1 }).toArray(function (err2, thumbnailData) {
                        if (err2) {
                            throw err2;
                        }
                        if (thumbnailData.length === 1) {
                            idObj_1 = thumbnailData[0]._id;
                        }
                        else {
                            idObj_1 = mongo_1.ObjectId(id_1);
                        }
                        var gridFSBucket = new mongo_1.GridFSBucket(fng.mongoose.connection.db, { bucketName: resource_1.model.collection.name });
                        var readstream = gridFSBucket.openDownloadStream(idObj_1);
                        readstream.pipe(res);
                    });
                });
            }
